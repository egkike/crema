...
- Nuevas tablas en `db/init/14-ai-insights-expansion.sql`: `churn_predictions`, `recovery_emails`, `ab_comparatives`
...
| DB Init | `backend/db/init/14-ai-insights-expansion.sql` | **Create** — Nuevas tablas `churn_predictions`, `recovery_emails`, `ab_comparatives` + ALTER `insights_history` |
| Types | `backend/src/types/ai.types.ts` (o nuevo) | **Possibly modify** — Interfaces `ChurnPrediction`, `RecoveryEmail`, `CompareResult` |
| Tests | `backend/src/__tests__/services/ai/agents.service.test.ts` | **Modify** — Agregar tests para nuevos métodos |
| Tests | `backend/src/__tests__/services/ai/ai-boot.test.ts` | **Modify** — Agregar tests de validación de handlers |
...
**Nota**: Un cuarto factor ("Acceso frecuente sin progreso") fue removido en v1 porque `total_orders` mide compras, no accesos. El tracking de actividad requiere infraestructura adicional.
...
| `backend/db/init/14-ai-insights-expansion.sql` | **New** | Agregar `churn_predictions`, `recovery_emails`, `ab_comparatives` (secciones 7.5, 7.6, 7.7) + ALTER `insights_history` |
| `backend/src/types/ai.types.ts` | **Possibly Modified** | Interfaces para ChurnPrediction, RecoveryEmail, CompareResult |
| `backend/src/__tests__/services/ai/agents.service.test.ts` | **Modified** | Tests unitarios para nuevos métodos |
| `backend/src/__tests__/services/ai/ai-boot.test.ts` | **Modified** | Tests de validación de handlers nuevos |
...
- DB init: `backend/db/init/14-ai-insights-expansion.sql` (nuevas tablas 7.5, 7.6, 7.7 + ALTER insights_history)
