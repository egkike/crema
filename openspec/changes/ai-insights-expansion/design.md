...
interface ChurnPrediction {
  ...
  riskFactors: string[];    // e.g. ["Inactivo 45 días", "Progreso < 20%"]
...
6. Save both SQL queries + results to `insights_history`
7. Deduct 3 credits
...
| 4 | Acceso frecuente sin progreso | `total_orders >= 3 AND days_since_last_activity <= 7 AND progress < 10%` | +10% |
...
Since `DOMPurify` is a browser library, for server-side we use a regex-based allowlist approach:
...
| 7 | `backend/src/__tests__/services/agents.service.test.ts` | **Modify** — Add tests for `predictChurn`, `generateRecoveryEmail`, `compareEntities` | +300 |
...
### 12.1 Unit Tests (`agents.service.test.ts`)
...
| `compareEntities` | Compares two products; compares two periods; throws 403 when not owner of compared product; throws 400 for invalid entityType; returns deltas with correct calculations; persists to `insights_history` |

### 12.2 Integration Tests (`ai.routes.test.ts`)
...
```typescript
// llmService mock pattern (existing in agents.service.test.ts)
...
| HTML sanitization library vs regex | Regex allowlist | DOMPurify is browser-only; backend regex blacklist is sufficient for LLM-generated HTML |
...
- Tests: `backend/src/__tests__/services/agents.service.test.ts`
