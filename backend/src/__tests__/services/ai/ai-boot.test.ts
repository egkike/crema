...
it('should register all 18 skills', async () => {
  await registerAISkills();
  expect(skillsRegistry.register).toHaveBeenCalledTimes(18);
  expect(registerCallCount).toBe(18);
});
...
it('memory.search handler requires query', async () => {
  ...
  await expect(handler({ query: '' })).rejects.toThrow('query is required and must be a non-empty string');
});
...
it('memory.search handler validates limit range', async () => {
  ...
  await expect(handler({ query: 'test', limit: 0 })).rejects.toThrow('limit must be a positive integer between 1 and 100');
  await expect(handler({ query: 'test', limit: 101 })).rejects.toThrow('limit must be a positive integer between 1 and 100');
});
...
it('memory.search handler validates sourceTypes', async () => {
  ...
  await expect(handler({ query: 'test', sourceTypes: 'invalid' })).rejects.toThrow('sourceTypes must be an array');
  await expect(handler({ query: 'test', sourceTypes: ['invalid'] })).rejects.toThrow('sourceTypes must be one of');
});
...
it('credits.balance handler requires requestingUserId and userId', async () => {
  ...
  // Missing userId
  await expect(handler({ requestingUserId: 'u1' })).rejects.toThrow('userId is required and must be a non-empty string');
});
...
it('credits.balance handler validates authorization', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', userId: 'u2' })).rejects.toThrow('Unauthorized access to user credits');
});
...
it('qa.chat handler requires requestingUserId', async () => {
  ...
  await expect(handler({ productId: 'p1', userId: 'u1', message: 'test' })).rejects.toThrow('requestingUserId is required');
});
...
it('qa.chat handler requires message', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', productId: 'p1', userId: 'u1', message: '' })).rejects.toThrow('message is required and must be a non-empty string');
});
...
it('qa.chat handler rejects message > 2000 chars', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', productId: 'p1', userId: 'u1', message: longMessage })).rejects.toThrow('message must be less than 2000 characters');
});
...
it('qa.chat handler validates authorization', async () => {
  ...
  // Mismatched IDs should fail with 403
  await expect(handler({ requestingUserId: 'u1', productId: 'p1', userId: 'u2', message: 'test' })).rejects.toThrow('Unauthorized access to user resource');
});
...
it('content.analyze handler requires analysisType', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', content: 'test', analysisType: 'invalid' })).rejects.toThrow();
});
...
it('content.analyze handler accepts valid analysisType', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', content: 'test content', analysisType: 'summary' })).resolves.toBeDefined();
  await expect(handler({ requestingUserId: 'u1', content: 'test content', analysisType: 'topics' })).resolves.toBeDefined();
  await expect(handler({ requestingUserId: 'u1', content: 'test content', analysisType: 'questions' })).resolves.toBeDefined();
  await expect(handler({ requestingUserId: 'u1', content: 'test content', analysisType: 'full' })).resolves.toBeDefined();
});
...
it('insights.ask handler requires requestingUserId', async () => {
  ...
  // Missing requestingUserId should fail
  await expect(handler({ userId: 'u1', naturalLanguageQuery: 'test' })).rejects.toThrow('requestingUserId is required');
});
...
it('insights.ask handler requires query', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', userId: 'u1', naturalLanguageQuery: '' })).rejects.toThrow('naturalLanguageQuery is required and must be a non-empty string');
});
...
it('insights.ask handler rejects query > 500 chars', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', userId: 'u1', naturalLanguageQuery: longQuery })).rejects.toThrow('naturalLanguageQuery must be less than 500 characters');
});
...
it('insights.ask handler validates authorization', async () => {
  ...
  // Mismatched IDs should fail with 403
  await expect(handler({ requestingUserId: 'u1', userId: 'u2', naturalLanguageQuery: 'test' })).rejects.toThrow('Unauthorized access to user insights');
});
...
it('analytics.metrics handler requires requestingUserId', async () => {
  ...
  // Missing requestingUserId should fail
  await expect(handler({ creatorId: 'creator1' })).rejects.toThrow('requestingUserId is required');
});
...
it('analytics.metrics handler validates authorization', async () => {
  ...
  // Mismatched IDs should fail with 403
  await expect(handler({ requestingUserId: 'u1', creatorId: 'creator2' })).rejects.toThrow('Unauthorized access to creator analytics');
});
...
it('analytics.metrics handler validates invalid date format', async () => {
  ...
  await expect(handler({ requestingUserId: 'creator1', creatorId: 'creator1', startDate: 'invalid' })).rejects.toThrow('startDate must be a valid ISO date string');
  await expect(handler({ requestingUserId: 'creator1', creatorId: 'creator1', startDate: '2024-13-01' })).rejects.toThrow('startDate must be a valid date');
});
...
it('analytics.metrics handler accepts optional dates', async () => {
  ...
  await expect(handler({ requestingUserId: 'creator1', creatorId: 'creator1', startDate: '2024-01-01', endDate: '2024-12-31' })).resolves.toBeDefined();
});
...
it('qa.list handler requires productId', async () => {
  ...
  await expect(handler({})).rejects.toThrow('productId is required and must be a non-empty string');
});
...
it('reports.create handler validates contentType', async () => {
  ...
  await expect(handler({ reporterId: 'u1', contentType: 'invalid', contentId: 'c1', reasonCode: 'spam' })).rejects.toThrow('contentType must be one of');
});
...
it('reports.create handler accepts valid contentType', async () => {
  ...
  for (const contentType of validTypes) {
    await expect(handler({ reporterId: 'u1', contentType, contentId: 'c1', reasonCode: 'spam' })).rejects.toThrow();
  }
});
...
it('reports.create handler requires reasonCode', async () => {
  ...
  await expect(handler({ reporterId: 'u1', contentType: 'product', contentId: 'c1', reasonCode: '' })).rejects.toThrow('reasonCode is required');
});
...
it('concierge-chat handler requires requestingUserId', async () => {
  ...
  // Missing requestingUserId should fail
  await expect(handler({ message: 'test', userId: 'u1' })).rejects.toThrow('requestingUserId is required');
});
...
it('concierge-chat handler validates authorization', async () => {
  ...
  // Mismatched IDs should fail with 403
  await expect(handler({ requestingUserId: 'u1', message: 'test', userId: 'u2' })).rejects.toThrow('Unauthorized access to user concierge');
});
...
it('memory.search handler rejects sourceTypes array > 20 items', async () => {
  ...
  await expect(handler({ query: 'test', sourceTypes: tooManyTypes })).rejects.toThrow('sourceTypes array exceeds maximum size of 20');
});
...
it('content.analyze handler rejects content > 50000 chars', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', content: longContent, analysisType: 'summary' })).rejects.toThrow('exceeds maximum length of 50000');
});
...
it('content.quiz handler rejects content > 50000 chars', async () => {
  ...
  const longContent = 'a'.repeat(50001);
  await expect(handler({ content: longContent, analysisType: 'quiz' })).rejects.toThrow('exceeds maximum length of 50000');
});
...
it('content.transcribe handler validates authorization', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', userId: 'u2', file: Buffer.from('test') })).rejects.toThrow('Unauthorized access to user transcription resource');
});
...
it('content.transcribe handler rejects buffer > 25MB', async () => {
  ...
  await expect(handler({ requestingUserId: 'u1', userId: 'u1', file: tooLargeBuffer })).rejects.toThrow('exceeds maximum size of 25MB');

  // Also test edge case with Buffer object directly (26MB)
  const buffer26MB = Buffer.alloc(26 * 1024 * 1024);
  await expect(handler({ requestingUserId: 'u1', userId: 'u1', file: buffer26MB })).rejects.toThrow('exceeds maximum size of 25MB');
});
});