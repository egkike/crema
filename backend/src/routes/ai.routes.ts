...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const validTypes: EmbeddingSourceType[] = [
        'lesson',
        'faq',
        'policy',
        'qa',
      ...
      const embedding = await memoryService.addEmbedding(
        userId,
        sourceType,
        sourceId,
        content,
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const userId = uid(req);
      const deleted = await memoryService.deleteEmbedding(
        sourceTypeStr as EmbeddingSourceType,
        sourceIdStr,
        userId
      );
...
router.get('/products/:productId/questions', aiLimiter, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const { questions, total } = await qaService.getQuestions(
      productId,
      showUnpublished,
      limit,
      offset
...
router.get('/products/:productId/reviews', aiLimiter, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const { reviews, total, avgRating } = await reviewService.getReviews(
      productId,
      includeUnpublished,
      limit,
      offset
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const userId = uid(req);
      const { allow_reviews, require_verified_purchase, auto_publish, min_rating, max_rating } =
        req.body;
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const userId = uid(req);
      const { is_enabled, model, system_prompt, temperature, max_tokens, use_memory, use_faqs } =
        req.body;
 ...
 */
router.post(
  '/products/:productId/tutor/chat/stream',
  jwtAuthMiddleware,
  aiChatLimiter,
  validate(chatMessageSchema),
  asyncHandler(async (req: Request, res: Response) => {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (productCheck.rows.length === 0) {
      res
        .status(403)
        .json({ error: 'You must be a creator with at least one product to use insights.' });
      return;
    }
        ...
        if (intent !== 'affiliate_metrics') {
          ...
          try {
            ...
            logger.error(
              ...
              'Credit deduction failed after LLM success — credits may be consumed but response delivered'
            );
          }
        }
      }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // PRE-CHECK: Verify user has enough credits BEFORE calling expensive LLM
    ...
    // STEP 1: Call LLM with timeout protection (fail-fast before credit deduction)
    const SEO_LLM_TIMEOUT_MS = 60_000; // 60 seconds
    const seoPromise = seoOptimizerService.generate({
      userId,
      productId,
      productName,
      productDescription,
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new AppError('SEO generation timed out', 504)), SEO_LLM_TIMEOUT_MS);
    });
    const result = await Promise.race([seoPromise, timeoutPromise]);
